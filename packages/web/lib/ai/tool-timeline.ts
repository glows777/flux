...
type ToolUIPartCandidate = Parameters<typeof isToolUIPart>[0]
...
export function groupPartsToSegments(parts: readonly unknown[]): Segment[] {
    ...
    for (let i = 0; i < parts.length; i++) {
        ...
        // Tool UI parts
        const toolPart = part as ToolUIPartCandidate
        if (isToolUIPart(toolPart)) {
...
export function buildTimelineSummary(steps: readonly TimelineStep[]): string {
    ...
    if (newsSteps.length > 0) {
        const total = newsSteps.reduce(
            (sum, s) => sum + countArray(s.output),
            0,
        )
    ...
    for (const s of okSteps) {
        if (s.toolName === 'webSearch')
            sourceCount += getSourcesCount(s.output) || 1
    ...
    if (sourceCount > 0) {
        ...
        if (lastIdx >= 0 && parts[lastIdx].includes('新闻')) {
            parts.splice(
                lastIdx,
                1,
                `${parts[lastIdx]}和 ${sourceCount} 个来源`,
            )
        } else {
    ...
    // Indicators
    const hasIndicators = okSteps.some(
        (s) => s.toolName === 'calculateIndicators',
    )
    if (hasIndicators) {
        parts.push('计算了技术指标')
    }
    ...
    for (const s of searchStockSteps) {
        const query =
            s.input && typeof s.input === 'object' && 'query' in s.input
                ? (s.input as Record<string, unknown>).query
                : undefined
        parts.push(
            typeof query === 'string' ? `搜索了 "${query}"` : '搜索了股票',
        )
    }
    ...
    const hasMemoryWrite = okSteps.some(
        (s) =>
            s.toolName === 'update_core_memory' || s.toolName === 'save_lesson',
    )
