...
const COMPLETION_HANDLERS: Record<string, (output: unknown) => string | null> = {
  ...
  calculateIndicators: (output) => {
    ...
    if (o.macd && typeof o.macd === 'object') {
      ...
      if (typeof macd.crossover === 'string') {
        const label = macd.crossover === 'golden' ? '金叉' : macd.crossover === 'death' ? '死叉' : null
  ...
  webFetch: (output) => {
    if (output && typeof output === 'object' && 'url' in output) {
      ...
      if (typeof url === 'string') return `已阅读 ${extractHostname(url)}`
    }
    return null
  },
}
 ...
 */
export function getCompletionSummary(toolName: string, output?: unknown): string | null {
