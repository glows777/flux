  ...
  return (
    <div className='my-3 rounded-xl border border-white/5 bg-white/[0.02] p-4'>
      ...
      <div className='flex items-center justify-between mb-3'>
        <span className='text-xs text-slate-400'>{data.symbol} 技术信号</span>
        <span className={`px-2 py-0.5 rounded text-xs border ${biasColor}`}>
      ...
      <div className='flex flex-wrap gap-2'>
        {data.signals.map((signal, i) => {
          const colorClasses = TYPE_COLORS[signal.type] ?? TYPE_COLORS.neutral
          return (
            <div
              key={i}
