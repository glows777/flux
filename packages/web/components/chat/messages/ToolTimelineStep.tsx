...
import {
  Loader2, Clock, X,
  DollarSign, Briefcase, Newspaper, BarChart2, Activity,
  FileText, Search, Globe, ExternalLink, File, Save, List,
} from 'lucide-react'
import type { LucideIcon } from 'lucide-react'
import type { TimelineStep } from '@/lib/ai/tool-timeline'
import { getLoadingLabel, getCompletionSummary } from '@/lib/ai/tool-display'
...
function StepIcon({ step }: { readonly step: TimelineStep }) {
  ...
  const IconComponent = step.toolName ? ICON_COMPONENTS[step.toolName] : undefined
  if (IconComponent) {
    return <IconComponent size={15} className='text-slate-500' />
  }
  return <div className='w-[15px] h-[15px] rounded-full border border-slate-600' />
}
...
function ThinkingStep({ step }: { readonly step: TimelineStep }) {
  return (
    <p className='text-[13px] text-slate-500 leading-relaxed m-0'>{step.text}</p>
  )
}

function WebSearchStep({ step }: { readonly step: TimelineStep }) {
  const query = getString(step.input, 'query')
  const sources = step.output && typeof step.output === 'object' && 'sources' in step.output
    ? (step.output as Record<string, unknown>).sources as Array<{ title: string; url: string; score: number }>
    : []
  ...
  return (
    <div>
      <div className='flex items-center justify-between'>
        ...
        {sources.length > 0 && (
          <span className='text-xs text-slate-600'>{sources.length} results</span>
        )}
      </div>
...
function MemorySearchStep({ step }: { readonly step: TimelineStep }) {
  const query = getString(step.input, 'query') ?? getString(step.input, 'symbol')
  const results = step.output && typeof step.output === 'object' && 'results' in step.output
    ? (step.output as Record<string, unknown>).results as Array<{ docPath: string; content: string; score: number; entities: readonly string[] }>
    : []
  ...
  return (
    <div>
      <div className='flex items-center justify-between'>
        ...
        {results.length > 0 && (
          <span className='text-xs text-slate-600'>{results.length} 条记录</span>
        )}
      </div>
...
function WebFetchStep({ step }: { readonly step: TimelineStep }) {
  ...
  const title = step.output && typeof step.output === 'object'
...
function MemoryReadStep({ step }: { readonly step: TimelineStep }) {
  ...
  const summary = step.state === 'done' ? getCompletionSummary(step.toolName ?? '', step.output) : null
  return (
    <div className='flex items-center gap-2'>
      ...
      {summary && (
        <>
          ...
          <span className='text-[13px] text-slate-300'>{summary}</span>
        </>
      )}
    </div>
  )
  ...
  return (
    <div className='flex items-center gap-2'>
      ...
      {summary && (
        <>
          <span className='text-slate-600 text-xs'>·</span>
          <span className='text-[13px] text-emerald-400'>{summary}</span>
        </>
      )}
    </div>
  )
...
function ErrorStep({ step }: { readonly step: TimelineStep }) {
  const label = getLoadingLabel(step.toolName ?? '', step.input)
  const truncated = step.errorText && step.errorText.length > 50
...
function StepContent({ step }: { readonly step: TimelineStep }) {
  ...
  switch (step.toolName) {
    case 'webSearch': return <WebSearchStep step={step} />
    case 'memory_search': return <MemorySearchStep step={step} />
    case 'webFetch': return <WebFetchStep step={step} />
    case 'memory_read':
    case 'memory_list': return <MemoryReadStep step={step} />
    case 'memory_write':
    case 'memory_append': return <MemoryWriteStep step={step} />
    default: return <DataToolStep step={step} />
  }
}
