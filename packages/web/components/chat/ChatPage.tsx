...
export function ChatPage() {
    ...
    // Session list (global)
    const { data: sessions, error: sessionsError, isLoading: sessionsLoading, mutate: mutateSessions } = useSWR<ChatSession[]>(
        '/api/sessions',
        fetcher,
    )
        ...
        <div className='flex h-full flex-1 min-w-0'>
            ...
            <div className='flex-1 flex flex-col min-w-0'>
                ...
                <div className='flex-1 overflow-y-auto scrollbar-thin scrollbar-thumb-white/10'>
                    {messages.length === 0 ? (
                        <ChatWelcome symbol={symbol} onSuggestionClick={handleSuggestionClick} />
                    ) : (
                        <div className='max-w-5xl w-[85%] mx-auto px-6 py-6 space-y-4'>
                            {messages.map((msg, index) => {
                                const cutoffIndex = messages.length - TRUNCATE_LIMIT
                                const showDivider = messages.length > TRUNCATE_LIMIT && index === cutoffIndex
                            ...
                            {error && <ErrorBanner error={error} onReload={regenerate} />}
                            <div ref={bottomRef} />
                        </div>
                    )}
                </div>
