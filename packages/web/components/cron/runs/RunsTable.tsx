                        ...
                        {runs.map((run) => (
                            ...
                            >
                                ...
                                <td className='px-4 py-3'>
                                    <span
                                        className={`px-2 py-0.5 rounded-full text-[10px] font-medium ${statusBadgeClass(run.status)}`}
                                    >
                                        {run.status}
                                    </span>
                                </td>
                                <td className='px-4 py-3 text-slate-500'>
                                    <span
                                        className={`px-1.5 py-0.5 rounded text-[10px] ${run.triggeredBy === 'manual' ? 'bg-blue-500/10 text-blue-400' : 'bg-white/5 text-slate-500'}`}
                                    >
                                        {run.triggeredBy}
                                    </span>
                                </td>
                ...
                <Dialog.Portal>
                    ...
                    <Dialog.Content className='fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-xl bg-[#111] border border-white/10 rounded-xl shadow-2xl z-50 p-6'>
                        ...
                        <div className='flex gap-3 mb-4'>
                            <span
                                className={`px-2 py-0.5 rounded-full text-[10px] font-medium ${statusBadgeClass(selectedRun?.status ?? '')}`}
                            >
                                {selectedRun?.status}
                            </span>
