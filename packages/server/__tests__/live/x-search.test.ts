...
import { clearXSearchCache, createSearchXTool } from '@/core/ai/research'
...
describe.skipIf(!HAS_XAI)('searchX tool (live)', () => {
    ...
    it('returns report and sources for a finance query', async () => {
        ...
        if (!result.report) throw new Error('Expected searchX report')
    ...
    it('supports handles filter', async () => {
        ...
        expect(typeof result.report).toBe('string')
        expect(result.report).toBeTruthy()
        if (!result.report) throw new Error('Expected handles-filter report')
    ...
    it('supports timeRange filter', async () => {
        ...
        expect(typeof result.report).toBe('string')
        expect(result.report).toBeTruthy()
        if (!result.report) throw new Error('Expected timeRange-filter report')
        expect(result.report.length).toBeGreaterThan(10)
    }, 120_000)
