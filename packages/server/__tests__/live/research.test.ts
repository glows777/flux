...
const HAS_SEARCH_MODEL = HAS_AI || !!(process.env.SEARCH_API_KEY && process.env.SEARCH_BASE_URL)
...
describe.skipIf(!HAS_AI)('webFetch tool (live)', () => {
  ...
  it('reads a page and returns summary', async () => {
    const result = await tools.webFetch.execute(
      {
        url: TEST_URL,
        question: 'What does NVIDIA do and what are their main products?',
      },
      toolCtx,
    ) as { url?: string; summary?: string; content?: string; bytesFetched?: number; source?: string; error?: string }
...
describe.skipIf(!HAS_TAVILY || !HAS_SEARCH_MODEL)('webSearch tool (live)', () => {
  const tools = createResearchTools()
  const toolCtx = {
