...
import { notifyTrade, notifyError, notifyOrderEvent } from '@/core/trading-agent/discord-hook'
import type { OrderEventNotification } from '@/core/trading-agent/types'

describe('Discord Hook', () => {
  const originalFetch = globalThis.fetch
  const mockFetch = mock(() => Promise.resolve(new Response('ok', { status: 204 })))

  beforeEach(() => {
    globalThis.fetch = mockFetch as any
    process.env.DISCORD_TRADING_WEBHOOK_URL = 'https://discord.com/api/webhooks/test'
  })
  ...
  it('sends trade notification with correct format', async () => {
    await notifyTrade({ symbol: 'NVDA', side: 'buy', qty: 10, price: 140.5, reasoning: 'MA crossover' })
  ...
  it('silently ignores when webhook URL is not configured', async () => {
    delete process.env.DISCORD_TRADING_WEBHOOK_URL
    await notifyTrade({ symbol: 'NVDA', side: 'buy', qty: 10, price: 140, reasoning: 'test' })
    expect(mockFetch).not.toHaveBeenCalled()
  })

  it('never throws even if fetch fails', async () => {
    globalThis.fetch = mock(() => Promise.reject(new Error('network error'))) as any
    await expect(notifyTrade({ symbol: 'NVDA', side: 'buy', qty: 10, price: 140, reasoning: 'test' })).resolves.toBeUndefined()
  })
