 ...
 *
 * 测试场景:
 * - T14-01: buildReportPrompt 包含所有数据
...
import { buildReportPrompt, buildAgentSystemPrompt, calculateIndicators } from '@/core/ai/prompts'
import type { EnhancedIndicators } from '@/core/ai/prompts'
import type { ReportContext } from '@/core/ai/types'
import type { HistoryPoint } from '@/core/market-data'

// ==================== 测试数据工厂 ====================

function createHistoryPoint(overrides: Partial<HistoryPoint> & { close: number }): HistoryPoint {
 ...
 */
function createIncreasingHistory(days: number, startPrice = 100): readonly HistoryPoint[] {
  return Array.from({ length: days }, (_, i) => createHistoryPoint({
    date: new Date(Date.now() - (days - i) * 86400000),
    close: startPrice + i,
 ...
 */
function createDecreasingHistory(days: number, startPrice = 200): readonly HistoryPoint[] {
  return Array.from({ length: days }, (_, i) => createHistoryPoint({
    date: new Date(Date.now() - (days - i) * 86400000),
    close: startPrice - i,
    open: startPrice - i + 0.5,
...
describe('P2-14: calculateIndicators', () => {
  it('T14-02: 正确计算 20 日均线 (MA20)', () => {
    ...
    const history = prices.map((close, i) => createHistoryPoint({
  ...
  it('T14-02b: MA20 仅使用最后 20 天数据', () => {
    const history = Array.from({ length: 30 }, (_, i) => createHistoryPoint({
...
describe('Enhanced: MA50/MA200', () => {
  it('MA50 正确计算 — 50 天数据', () => {
    const history = Array.from({ length: 50 }, (_, i) => createHistoryPoint({
  ...
  it('MA200 正确计算 — 200 天数据', () => {
    const history = Array.from({ length: 200 }, (_, i) => createHistoryPoint({
...
describe('Enhanced: trendPosition', () => {
  it('上升趋势 — price > MA20 > MA50 > MA200 → above-all', () => {
    ...
    const history: HistoryPoint[] = [
      ...Array.from({ length: 150 }, (_, i) => createHistoryPoint({
        date: new Date(Date.now() - (200 - i) * 86400000),
        close: 50, // 低价 → 拉低 MA200
      })),
      ...Array.from({ length: 30 }, (_, i) => createHistoryPoint({
        date: new Date(Date.now() - (50 - i) * 86400000),
        close: 100, // 中价 → MA50 中
      })),
      ...Array.from({ length: 20 }, (_, i) => createHistoryPoint({
        date: new Date(Date.now() - (20 - i) * 86400000),
...
describe('Enhanced: MACD', () => {
  ...
  it('MACD golden cross — 近 3 日 MACD 由下穿上 signal', () => {
    ...
    const history: HistoryPoint[] = [
      ...
      ...Array.from({ length: 10 }, (_, i) => createHistoryPoint({
...
describe('Enhanced: support/resistance', () => {
  it('support 取近 20 日最低价', () => {
    const history = Array.from({ length: 20 }, (_, i) => createHistoryPoint({
      date: new Date(Date.now() - (20 - i) * 86400000),
      close: 100 + i,
...
describe('Enhanced: volumeRatio', () => {
  it('放量 — 近 5 日均量 > 近 20 日均量 → >1', () => {
    const history: HistoryPoint[] = [
      // 前 15 天低量
      ...Array.from({ length: 15 }, (_, i) => createHistoryPoint({
        date: new Date(Date.now() - (20 - i) * 86400000),
        close: 100,
        volume: 1000,
      })),
      // 后 5 天高量
      ...Array.from({ length: 5 }, (_, i) => createHistoryPoint({
...
describe('P2-14: buildReportPrompt', () => {
  it('T14-01: Prompt 包含 symbol, name, price, change', () => {
  ...
  it('T14-01b: Prompt 包含基本面数据', () => {
    const context = createMockContext()
    const prompt = buildReportPrompt(context)

    expect(prompt).toContain('28.50')
    expect(prompt).toContain('$3000.00B')
    expect(prompt).toContain('6.15')
    expect(prompt).toContain('0.50%')
    expect(prompt).toContain('Technology')
  })

  it('T14-01c: Prompt 包含技术面数据', () => {
    const context = createMockContext()
    const prompt = buildReportPrompt(context)

    expect(prompt).toContain('MA20')
    expect(prompt).toContain('RSI')
  })

  it('T14-01d: Prompt 包含输出结构要求', () => {
    const context = createMockContext()
    const prompt = buildReportPrompt(context)

    expect(prompt).toContain('核心观点')
    expect(prompt).toContain('技术面分析')
    expect(prompt).toContain('基本面分析')
    expect(prompt).toContain('风险提示')
  })

  it('T14-05: 涨跌幅正数带 + 号', () => {
    const context = createMockContext({ change: 2.5 })
    const prompt = buildReportPrompt(context)
  ...
  it('T14-05b: 涨跌幅负数带 - 号', () => {
    const context = createMockContext({ change: -3.2 })
    const prompt = buildReportPrompt(context)
  ...
  it('基本面数据缺失时显示数据暂缺', () => {
    ...
    expect(prompt).toContain('数据暂缺')
  })

  it('RSI 超买区域提示', () => {
    const history = createIncreasingHistory(20)
    const context = createMockContext({ history })
    const prompt = buildReportPrompt(context)

    expect(prompt).toContain('RSI 超买区域')
  })
  ...
  it('RSI 超卖区域提示', () => {
    ...
    expect(prompt).toContain('RSI 超卖区域')
  })
})
