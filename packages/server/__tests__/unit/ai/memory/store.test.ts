...
describe('getSlotContent', () => {
    it('returns content when slot has a version', async () => {
        mockDb.memoryVersion.findFirst.mockResolvedValueOnce({
            content: '偏好成长股',
        })
...
describe('writeSlot', () => {
    it('inserts a new MemoryVersion record', async () => {
        await writeSlot('user_profile', 'A'.repeat(100), 'agent', '更新偏好', {
            db: mockDb,
        })
    ...
    it('inserts with reason=null when reason is omitted', async () => {
        await writeSlot('lessons', '不要追涨', 'agent', undefined, {
            db: mockDb,
        })
    ...
    it('throws SlotContentTooLongError when content exceeds limit', async () => {
        ...
        await expect(
            writeSlot('user_profile', tooLong, 'agent', undefined, {
                db: mockDb,
            }),
    ...
    it('throws SlotContentTooLongError with correct slot and limit in message', async () => {
        ...
        try {
            await writeSlot('portfolio_thesis', tooLong, 'agent', undefined, {
                db: mockDb,
            })
    ...
    it('accepts content exactly at the limit', async () => {
        ...
        await writeSlot('user_profile', atLimit, 'user', undefined, {
            db: mockDb,
        })
...
describe('getSlotHistory', () => {
    ...
    it('returns empty array when no versions exist', async () => {
        ...
        const result = await getSlotHistory('agent_strategy', 10, {
            db: mockDb,
        })
