...
describe('getLoadingLabel', () => {
    ...
    it('getQuote with symbol', () => {
        expect(getLoadingLabel('getQuote', { symbol: 'AAPL' })).toBe(
            '查询 AAPL 报价',
        )
    })

    it('getCompanyInfo with symbol', () => {
        expect(getLoadingLabel('getCompanyInfo', { symbol: 'TSLA' })).toBe(
            '获取 TSLA 公司信息',
        )
    })
    ...
    it('getNews with symbol', () => {
        expect(getLoadingLabel('getNews', { symbol: 'MSFT' })).toBe(
            '搜索 MSFT 新闻',
        )
    })
    ...
    it('getHistory with symbol and days', () => {
        expect(
            getLoadingLabel('getHistory', { symbol: 'AAPL', days: 90 }),
        ).toBe('获取 AAPL 近 90 天历史')
    })

    it('getHistory with symbol only', () => {
        expect(getLoadingLabel('getHistory', { symbol: 'AAPL' })).toBe(
            '获取 AAPL 历史数据',
        )
    })

    it('calculateIndicators with symbol', () => {
        expect(getLoadingLabel('calculateIndicators', { symbol: 'NVDA' })).toBe(
            '计算 NVDA 技术指标',
        )
    })

    it('searchStock with query', () => {
        expect(getLoadingLabel('searchStock', { query: '英伟达' })).toBe(
            '搜索 "英伟达"',
        )
    })
    ...
    it('update_core_memory with slot', () => {
        expect(
            getLoadingLabel('update_core_memory', { slot: 'market_views' }),
        ).toBe('更新 market_views 记忆')
    })
    ...
    it('read_history with slot', () => {
        expect(getLoadingLabel('read_history', { slot: 'lessons' })).toBe(
            '回顾 lessons 历史',
        )
    })
    ...
    it('webSearch with query', () => {
        expect(
            getLoadingLabel('webSearch', { query: 'AAPL Q1 earnings' }),
        ).toBe('搜索 "AAPL Q1 earnings"')
    })
    ...
    it('webFetch with url extracts hostname', () => {
        expect(
            getLoadingLabel('webFetch', {
                url: 'https://www.reuters.com/article/123',
            }),
        ).toBe('阅读 www.reuters.com')
    })
...
describe('getCompletionSummary', () => {
    ...
    it('returns error message when output has error key', () => {
        expect(
            getCompletionSummary('getQuote', {
                error: 'API rate limit exceeded',
            }),
        ).toBe('失败: API rate limit exceeded')
    })
    ...
    it('getQuote returns formatted price and change', () => {
        const result = getCompletionSummary('getQuote', {
            price: 198.5,
            change: 1.2,
        })
    ...
    it('getCompanyInfo returns name and PE', () => {
        const result = getCompletionSummary('getCompanyInfo', {
            name: 'Apple Inc.',
            pe: 32.1,
        })
    ...
    it('getHistory counts data points', () => {
        ...
        expect(getCompletionSummary('getHistory', points)).toBe(
            '已获取 90 天历史数据',
        )
    })

    it('calculateIndicators shows RSI and MACD golden cross', () => {
        const result = getCompletionSummary('calculateIndicators', {
            rsi: 62.3,
    ...
    it('searchStock counts results', () => {
        ...
        expect(getCompletionSummary('searchStock', results)).toBe(
            '找到 3 个匹配',
        )
    })
    ...
    it('update_core_memory returns slot name from message', () => {
        expect(
            getCompletionSummary('update_core_memory', {
                success: true,
                slot: 'lessons',
                message: '已更新 lessons',
            }),
    ...
    it('save_lesson returns fixed label', () => {
        expect(getCompletionSummary('save_lesson', { success: true })).toBe(
            '已记录教训',
        )
    })
    ...
    it('read_history counts versions', () => {
        ...
        expect(getCompletionSummary('read_history', output)).toBe(
            '找到 2 条历史',
        )
    })
    ...
    it('webFetch extracts hostname from output url', () => {
        ...
        expect(getCompletionSummary('webFetch', output)).toBe(
            '已阅读 www.reuters.com',
        )
    })
    ...
    it('display_rating_card returns null', () => {
        expect(
            getCompletionSummary('display_rating_card', { rating: 'BUY' }),
        ).toBeNull()
    })

    it('display_comparison_table returns null', () => {
        expect(
            getCompletionSummary('display_comparison_table', { rows: [] }),
        ).toBeNull()
    })

    it('display_signal_badges returns null', () => {
        expect(
            getCompletionSummary('display_signal_badges', { signals: [] }),
        ).toBeNull()
    })

    it('unknown tool returns null', () => {
        expect(
            getCompletionSummary('unknownTool', { data: 'something' }),
        ).toBeNull()
    })
})
