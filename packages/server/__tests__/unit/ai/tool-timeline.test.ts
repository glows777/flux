...
import type { Segment } from '@/core/ai/tool-timeline'
...
describe('getFaviconUrl', () => {
  it('returns google favicon URL for valid URL', () => {
    expect(getFaviconUrl('https://seekingalpha.com/article/123'))
      .toBe('https://www.google.com/s2/favicons?domain=seekingalpha.com&sz=32')
  })
...
describe('groupPartsToSegments', () => {
  ...
  it('groups consecutive tool parts into one timeline', () => {
    const parts = [
      ...
      { type: 'dynamic-tool' as const, toolCallId: 'b', toolName: 'getNews', state: 'output-available', output: [{ title: 'News' }], input: { symbol: 'AAPL' } },
    ]
    const segments = groupPartsToSegments(parts as any)
  ...
  it('text part closes current timeline', () => {
    const parts = [
      { type: 'dynamic-tool' as const, toolCallId: 'a', toolName: 'getQuote', state: 'output-available', output: { price: 100 }, input: {} },
      { type: 'text' as const, text: 'Here is the result' },
      { type: 'dynamic-tool' as const, toolCallId: 'b', toolName: 'getNews', state: 'output-available', output: [], input: {} },
    ]
    const segments = groupPartsToSegments(parts as any)
  ...
  it('display tool creates separate display segment', () => {
    const parts = [
      { type: 'dynamic-tool' as const, toolCallId: 'a', toolName: 'getQuote', state: 'output-available', output: {}, input: {} },
      { type: 'dynamic-tool' as const, toolCallId: 'b', toolName: 'display_rating_card', state: 'output-available', output: { symbol: 'AAPL', rating: '买入' }, input: {} },
    ]
    const segments = groupPartsToSegments(parts as any)
  ...
  it('preserves thinking within timelines that have tool steps', () => {
    const parts = [
      ...
      { type: 'dynamic-tool' as const, toolCallId: 'b', toolName: 'webSearch', state: 'output-available', output: { sources: [] }, input: { query: 'test' } },
    ]
    const segments = groupPartsToSegments(parts as any)
    expect(segments).toHaveLength(1)
    if (segments[0].type === 'timeline') {
  ...
  it('sets collapsed false when timeline has running steps', () => {
    const parts = [
      { type: 'dynamic-tool' as const, toolCallId: 'a', toolName: 'getQuote', state: 'input-streaming', input: {} },
    ]
    const segments = groupPartsToSegments(parts as any)
    if (segments[0].type === 'timeline') {
  ...
  it('maps SDK states correctly in steps', () => {
    const parts = [
      { type: 'dynamic-tool' as const, toolCallId: 'a', toolName: 'getQuote', state: 'output-error', input: {}, errorText: 'Timeout' },
    ]
    const segments = groupPartsToSegments(parts as any)
    if (segments[0].type === 'timeline') {
...
function makeStep(toolName: string, overrides?: Partial<TimelineStep>): TimelineStep {
...
describe('buildTimelineSummary', () => {
  ...
  it('appends error count', () => {
    ...
    expect(buildTimelineSummary(steps)).toBe('查询了 AAPL 报价。（1 项失败）')
  })
})
