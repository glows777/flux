...
const mockProxyFetch = mock(() =>
    Promise.resolve(
        new Response(
            `${'<html><body><h1>Title</h1><p>Some long content here repeated enough to pass minimum length check. '.repeat(
                20,
            )}</p></body></html>`,
            {
                status: 200,
...
const mockTurndownConvert = mock(
    () =>
        '# Title\n\n' +
        'Some long content here repeated enough to pass minimum length check. '.repeat(
            20,
        ),
)
...
const envKeys = ['TAVILY_API_KEY', 'JINA_API_KEY']
...
describe('createSummarize uses getModel("light")', () => {
    ...
    it('summarize invocation calls getModel("light")', async () => {
        const mockGenerateText = mock(() =>
            Promise.resolve({ text: 'summary result' }),
        )
...
describe('readPage implementation (defaultReadPage)', () => {
    ...
    const longParagraph =
        'This is a detailed paragraph with enough text content. '.repeat(15)
    ...
    beforeEach(() => {
        ...
        DOMParser.prototype.parseFromString = (str: string, _type: string) =>
            domino.createDocument(str)
        ...
        mockProxyFetch.mockImplementation(() =>
            Promise.resolve(makeHtmlResponse(defaultHtml)),
        )
    })
    afterEach(() => {
    ...
    it('direct fetch content too short (< 500 chars) -> falls back to Jina', async () => {
        mockProxyFetch
            .mockImplementationOnce(() =>
                Promise.resolve(
                    makeHtmlResponse('<html><body><p>Short</p></body></html>'),
                ),
            )
            .mockImplementationOnce(() =>
                Promise.resolve(
    ...
    it('Jina Reader URL format: https://r.jina.ai/{encoded url}', async () => {
        mockProxyFetch
            ...
            .mockImplementationOnce(() =>
                Promise.resolve(
                    new Response('# Jina content', { status: 200 }),
                ),
            )
        ...
        expect(jinaCall[0]).toBe(
            `https://r.jina.ai/${encodeURIComponent('https://example.com/page?q=test')}`,
        )
    })
    ...
    it('Jina request includes Accept: text/markdown header', async () => {
        ...
        const jinaCall = mockProxyFetch.mock.calls[1]
        const headers = (jinaCall[1] as { headers?: Record<string, string> })
            ?.headers
...
describe('searchWeb implementation', () => {
    ...
    it('passes query and options to tavily.search', async () => {
        ...
        expect(call[1]).toEqual(
            expect.objectContaining({ topic: 'finance', maxResults: 3 }),
        )
    })
