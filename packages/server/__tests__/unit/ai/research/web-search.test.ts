...
import {
    createSearchTavilyTool,
    createWebSearchTool,
} from '@/core/ai/research/web-search'

function makeMockDeps() {
    return {
        searchWeb: mock<
            (query: string, options?: unknown) => Promise<SearchResponse>
        >(() =>
            Promise.resolve({
                results: [
                    {
...
describe('createSearchTavilyTool', () => {
    ...
    it('calls deps.searchWeb with correct parameters', async () => {
        const t = createSearchTavilyTool(deps)
        await t.execute(
            { query: 'NVDA earnings', topic: 'finance', maxResults: 5 },
            {
                toolCallId: '1',
                messages: [],
    ...
    it('returns { error } when deps.searchWeb throws', async () => {
        deps.searchWeb.mockImplementation(() =>
...
describe('createWebSearchTool', () => {
    ...
    describe('generateText call parameters', () => {
        it('model is deps.searchModel', async () => {
            ...
            const call = deps.generateText.mock.calls[0][0] as Record<
                string,
                unknown
            >
