...
const mockGenerateText = mock(() =>
  Promise.resolve({
    ...
    sources: [
      { sourceType: 'url', url: 'https://x.com/i/status/123', title: 'Tweet about TSLA' },
      { sourceType: 'url', url: 'https://reuters.com/tsla', title: 'Reuters TSLA' },
    ],
  }),
)
...
mock.module('ai', () => ({
  tool: (config: unknown) => {
    const c = config as { description: string; inputSchema: unknown; execute: Function }
    return { description: c.description, execute: c.execute, inputSchema: c.inputSchema }
  },
...
import { createSearchXTool, clearXSearchCache, getXSearchCacheSize } from '@/core/ai/research/x-search'
...
describe('createSearchXTool', () => {
  ...
  describe('API key missing', () => {
    it('returns error when XAI_API_KEY is not set', async () => {
      ...
      const t = createSearchXTool()
      const result = await t.execute({ query: 'TSLA sentiment' }, toolCtx) as { error: string }
  ...
  describe('generateText call parameters', () => {
    ...
    it('passes handles as allowedXHandles when provided', async () => {
      const t = createSearchXTool()
      await t.execute({ query: 'test', handles: ['elonmusk', 'jimcramer'] }, toolCtx)

      const xSearchCall = mockXSearch.mock.calls[0][0] as Record<string, unknown>
      expect(xSearchCall.allowedXHandles).toEqual(['elonmusk', 'jimcramer'])
    })

    it('passes timeRange as fromDate/toDate when provided', async () => {
      const t = createSearchXTool()
      await t.execute(
        { query: 'test', timeRange: { from: '2026-03-01', to: '2026-03-27' } },
        toolCtx,
      )
  ...
  describe('return value', () => {
    it('report comes from result.text', async () => {
      const t = createSearchXTool()
      const result = await t.execute({ query: 'test' }, toolCtx) as { report: string }

      expect(result.report).toBe('X sentiment report: TSLA is trending bullish on social media.')
    })
    ...
    it('sources deduped by URL', async () => {
      mockGenerateText.mockImplementation(() =>
        Promise.resolve({
          text: 'report',
          sources: [
            { sourceType: 'url', url: 'https://x.com/1', title: 'T1' },
            { sourceType: 'url', url: 'https://x.com/1', title: 'T1 dup' },
            { sourceType: 'url', url: 'https://x.com/2', title: 'T2' },
          ],
        }),
      )
      const t = createSearchXTool()
  ...
  describe('error handling', () => {
    it('generateText throws -> returns { error }', async () => {
      ...
      const t = createSearchXTool()
      const result = await t.execute({ query: 'test' }, toolCtx) as { error: string }
