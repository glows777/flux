...
import type { AIPlugin, HookContext, AfterChatContext, ChatParams } from '../../../../src/core/ai/runtime/types'
...
describe('collectSystemPrompts', () => {
  ...
  test('calls dynamic prompt functions with context', async () => {
    ...
    const result = await collectSystemPrompts(plugins, { ...baseCtx, symbol: 'AAPL' })
...
describe('collectTools', () => {
  test('merges tools from multiple plugins', async () => {
    const plugins: AIPlugin[] = [
      { name: 'a', tools: { toolA: { tool: {} as any } } },
      { name: 'b', tools: { toolB: { tool: {} as any } } },
    ]
  ...
  test('throws ToolConflictError on duplicate tool names', async () => {
    ...
    expect(collectTools(plugins, baseCtx)).rejects.toBeInstanceOf(ToolConflictError)
  })

  test('supports dynamic tool functions', async () => {
    const plugins: AIPlugin[] = [
      { name: 'a', tools: async () => ({ dynamic: { tool: {} as any } }) },
    ]
    const result = await collectTools(plugins, baseCtx)
...
describe('runTransformChain', () => {
  test('applies transforms in plugin order', async () => {
    const plugins: AIPlugin[] = [
      { name: 'a', transformParams: (_ctx, p) => ({ ...p, maxSteps: 50 }) },
      { name: 'b', transformParams: (_ctx, p) => ({ ...p, temperature: 0.7 }) },
    ]
    const initial: ChatParams = { maxSteps: 20 }
    const result = await runTransformChain(plugins, 'transformParams', baseCtx, initial)
  ...
  test('skips plugins without the hook', async () => {
    const plugins: AIPlugin[] = [
      { name: 'a' },
      { name: 'b', transformParams: (_ctx, p) => ({ ...p, maxSteps: 99 }) },
    ]
    const initial: ChatParams = { maxSteps: 20 }
    const result = await runTransformChain(plugins, 'transformParams', baseCtx, initial)
  ...
  test('propagates errors as PluginError', async () => {
    const plugins: AIPlugin[] = [
      { name: 'bad', transformParams: () => { throw new Error('boom') } },
    ]
    const initial: ChatParams = { maxSteps: 20 }
    expect(runTransformChain(plugins, 'transformParams', baseCtx, initial))
...
describe('runAfterChatHooks', () => {
  test('calls all hooks in parallel', async () => {
    const order: string[] = []
    const plugins: AIPlugin[] = [
      { name: 'a', afterChat: async () => { order.push('a') } },
      { name: 'b', afterChat: async () => { order.push('b') } },
    ]
    const ctx = { ...baseCtx, result: { text: '', usage: { promptTokens: 0, completionTokens: 0 }, toolCalls: [] }, responseMessage: {} as any, toolCalls: [] } as AfterChatContext
  ...
  test('does not throw when a hook fails', async () => {
    const plugins: AIPlugin[] = [
      { name: 'bad', afterChat: async () => { throw new Error('fail') } },
