...
import { getQuoteWithCache, getInfoWithCache, getHistoryWithCache } from '../src/core/market-data/sync'
import { getStockNews } from '../src/core/market-data/news'
import { getReportFromCache } from '../src/core/ai/cache'
...
async function main() {
  ...
  if (!alpacaClient.isConfigured()) {
    console.error('Alpaca is not configured. Set ALPACA_KEY_ID and ALPACA_SECRET_KEY.')
    process.exit(1)
  }

  const account = await alpacaClient.getAccount()
  console.log('Alpaca account:', account ? `$${account.equity} equity` : 'FAILED')
  ...
  // Debug: check tools
  const { createTradingAgentTools } = await import('../src/core/trading-agent/tools')
  ...
  console.log('First tool type:', typeof firstTool, 'has execute:', typeof firstTool?.execute)

  // Debug: test individual tools
  console.log('\n--- Testing tools directly ---')
  try {
    const memResult = await tools.memory_read.execute({ path: 'trading-agent/strategy.md' }, { toolCallId: 'test', messages: [], abortSignal: undefined as any })
    console.log('memory_read:', JSON.stringify(memResult).slice(0, 200))
  } catch (e) { console.error('memory_read ERROR:', e) }

  try {
    const pfResult = await tools.getPortfolio.execute({}, { toolCallId: 'test', messages: [], abortSignal: undefined as any })
    console.log('getPortfolio:', JSON.stringify(pfResult).slice(0, 200))
  } catch (e) { console.error('getPortfolio ERROR:', e) }
