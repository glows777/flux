...
} from '@/core/broker/portfolio-calc'
import { findVixFromMacro, getInfo, getMacro } from '@/core/market-data'

const dashboard = new Hono().get('/', async (c) => {
    ...
    const watchlist =
        watchlistResult.status === 'fulfilled' ? watchlistResult.value : null
    const account =
    ...
    if (account) {
        ...
        const holdingItems: HoldingItem[] = positions.map((p, i) => {
            const name =
                nameResults[i].status === 'fulfilled'
                    ? (nameResults[i].value.name ?? null)
                    : null
        ...
        portfolio = {
            ...
            summary: calculateSummary(holdingItems, vix),
        }
    }
