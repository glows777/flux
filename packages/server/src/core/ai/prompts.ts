 ...
 * 计算技术指标
 */
export function calculateIndicators(history: readonly HistoryPoint[]): EnhancedIndicators {
...
export function buildReportPrompt(context: ReportContext): string {
  const { ma20, rsi } = calculateIndicators(context.history)

  const priceChange = context.change >= 0
    ? `+${context.change.toFixed(2)}%`
    : `${context.change.toFixed(2)}%`

  const metricsText = [
    context.metrics.pe ? `市盈率: ${context.metrics.pe.toFixed(2)}` : null,
    context.metrics.marketCap ? `市值: $${(context.metrics.marketCap / 1e9).toFixed(2)}B` : null,
    context.metrics.eps ? `每股收益: $${context.metrics.eps.toFixed(2)}` : null,
    context.metrics.dividendYield ? `股息率: ${(context.metrics.dividendYield * 100).toFixed(2)}%` : null,
    context.metrics.sector ? `行业: ${context.metrics.sector}` : null,
  ].filter(Boolean).join('\n')

  const technicalText = [
    ma20 ? `MA20: $${ma20.toFixed(2)}` : null,
    rsi != null ? `RSI(14): ${rsi}` : null,
    ma20 ? (context.price > ma20 ? '当前价格在 MA20 上方' : '当前价格在 MA20 下方') : null,
    rsi != null && rsi > 70 ? 'RSI 超买区域' : rsi != null && rsi < 30 ? 'RSI 超卖区域' : rsi != null ? 'RSI 中性区域' : null,
  ].filter(Boolean).join('\n')

  return `你是一位华尔街资深交易员，专注于股票分析。请基于以下数据生成一份简洁的中文投资研报。

## 股票信息
- 代码: ${context.symbol}
- 名称: ${context.name}
- 当前价格: $${context.price.toFixed(2)}
- 今日涨跌: ${priceChange}
...
请按以下结构输出研报 (使用 Markdown 格式):

## 核心观点
(用 1-2 句话总结投资建议)

## 技术面分析
(基于 MA20、RSI 等指标分析当前走势)
