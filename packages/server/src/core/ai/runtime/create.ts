  ...
  for (const plugin of plugins) {
    ...
    for (const toolName of Object.keys(plugin.tools)) {
      if (ownership[toolName] != null) {
        throw new ToolConflictError(toolName, ownership[toolName], plugin.name)
      }
...
export async function createAIRuntime(options: RuntimeOptions): Promise<AIRuntime> {
    ...
    try {
      ...
      if (transformedParams.thinkingBudget != null) {
        providerOptions.anthropic = {
          thinking: { type: 'enabled', budgetTokens: transformedParams.thinkingBudget },
        }
      }
      ...
      const resolvedMaxTokens = transformedParams.maxTokens
      ...
      const streamResult = streamText({
        ...
        ...(resolvedMaxTokens != null ? { maxOutputTokens: resolvedMaxTokens } : {}),
      ...
      let finalizedData: { text: string; usage: FinishedResult['usage']; toolCalls: ToolCallRecord[] } | null = null
      ...
      async function consumeStream(): Promise<ConsumedResult> {
        ...
        if (capturedResponseMessage == null) {
          throw new Error('Stream finished without producing a responseMessage')
        }
        ...
        return { text, responseMessage: capturedResponseMessage, toolCalls, usage }
      }
