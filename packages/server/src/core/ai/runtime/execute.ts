...
export async function collectSystemPrompts(
  ...
  return fragments.filter((f): f is string => f != null && f.length > 0).join('\n\n')
}
...
export async function collectTools(
  ...
  const pluginToolPairs = await Promise.all(
    plugins.map(async (plugin) => {
      if (plugin.tools == null) return { name: plugin.name, tools: {} as ToolMap }
      try {
        const tools = typeof plugin.tools === 'function'
...
export async function runTransformChain<T>(
  ...
  let current = initial
  for (const plugin of plugins) {
    const hook = plugin[hookName] as ((ctx: HookContext, value: T) => T | Promise<T>) | undefined
