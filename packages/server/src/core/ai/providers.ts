...
const SLOTS: Record<ModelSlot, SlotConfig> = {
  main:      { provider: 'anthropic', envKey: 'MAIN_MODEL',      defaultModel: 'claude-sonnet-4-6' },
  search:    { provider: 'anthropic', envKey: 'SEARCH_MODEL',    defaultModel: 'claude-opus-4-6' },
  light:     { provider: 'openai',    envKey: 'LIGHT_MODEL',     defaultModel: 'google/gemini-2.5-flash' },
  embedding: { provider: 'embedding', envKey: 'EMBEDDING_MODEL', defaultModel: 'google/gemini-embedding-001' },
  xSearch:   { provider: 'xai',       envKey: 'XSEARCH_MODEL',   defaultModel: 'grok-4-1-fast' },
} as const
...
function createProviderInstance(key: ProviderKey): ProviderInstance {
  ...
  switch (key) {
    ...
    case 'embedding': {
      ...
      if (!apiKey) throw new Error('EMBEDDING_API_KEY (or OPENAI_API_KEY) is not configured')
      if (!baseURL) throw new Error('EMBEDDING_BASE_URL (or OPENAI_BASE_URL) is not configured')
      return createOpenAI({ apiKey, baseURL, fetch: fetchOpt })
    }
    case 'xai': {
      ...
      return createXai({
        apiKey,
        ...(process.env.XAI_BASE_URL ? { baseURL: process.env.XAI_BASE_URL } : {}),
        fetch: xaiFetch,
      })
    }
  }
...
export function getModel(
  ...
  if (!isModelSlot(slotOrProvider)) {
    throw new Error(`Invalid model slot: ${slotOrProvider}. Use one of: ${MODEL_SLOTS.join(', ')}`)
  }
  ...
  if (config.provider === 'xai') {
    return (provider as ReturnType<typeof createXai>).responses(resolvedModelId)
  }
...
export function getEmbeddingModel(): EmbeddingModel {
  ...
  const resolvedModelId = process.env[config.envKey] || config.defaultModel
  const provider = getProviderInstance('embedding') as ReturnType<typeof createOpenAI>
