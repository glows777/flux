...
export function createSearchTavilyTool(deps: Pick<ResearchDeps, 'searchWeb'>) {
    return tool({
        description:
        ...
        execute: async ({
            ...
            try {
                ...
                const response = await deps.searchWeb(
                    query,
                    options as Parameters<ResearchDeps['searchWeb']>[1],
                )
                return response
            } catch (error) {
                return {
        ...
        execute: async ({ query }, { abortSignal }) => {
            try {
                ...
                const result = (await deps.generateText({
                    ...
                    typeof deps.generateText
                >[0])) as unknown as GenerateTextResult
