...
function extractCitations(sources: GenerateTextSource[] | undefined): XSearchSource[] {
  ...
  for (let i = 0; i < sources.length; i++) {
    ...
    seen.set(s.url, { title, url: s.url, score: Math.round(score * 100) / 100 })
  }
...
export function createSearchXTool() {
  return tool({
    ...
    execute: async ({ query, timeRange, handles }, { abortSignal }): Promise<XSearchResult> => {
      // Check API key before doing anything
      if (!process.env.XAI_API_KEY) {
        return { error: 'X search unavailable: XAI_API_KEY not configured' }
      }
      ...
      try {
        ...
        const tools = {
          ...
          x_search: xai.tools.xSearch({
            ...
            ...(timeRange?.from ? { fromDate: timeRange.from } : {}),
            ...(timeRange?.to ? { toDate: timeRange.to } : {}),
            enableImageUnderstanding: X_SEARCH_CONFIG.enableImageUnderstanding,
            enableVideoUnderstanding: X_SEARCH_CONFIG.enableVideoUnderstanding,
          }),
