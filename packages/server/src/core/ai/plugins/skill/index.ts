...
export function skillPlugin(options: SkillPluginOptions): AIPlugin {
  ...
  let skillTool: unknown | null = null
  let bashTools: Record<string, unknown> | null = null
  ...
  return {
    ...
    async init() {
      try {
        ...
        console.error('[skill-plugin] Init failed, plugin disabled:', error)
      }
    },
    ...
    tools(): ToolMap {
      ...
      return {
        loadSkill: { tool: skillTool as any },
        bash: { tool: (bashTools as any).bash },
        readFile: { tool: (bashTools as any).readFile },
        writeFile: { tool: (bashTools as any).writeFile },
      }
    },
  }
}
