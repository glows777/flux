...
export function heartbeatPlugin(deps: HeartbeatPluginDeps): AIPlugin {
  ...
  return {
    ...
    async beforeChat(ctx: HookContext): Promise<void> {
      ...
      try {
        const openOrders = await alpacaClient.getOrders({ status: 'open', limit: 50 })
        for (const order of openOrders) {
          const existing = await (db as any).order.findUnique({ where: { alpacaOrderId: order.id } })
          if (existing) {
            if ((existing as { status: string }).status !== order.status) {
            ...
            await (db as any).order.create({
              data: {
                ...
                type: order.type, timeInForce: order.timeInForce ?? 'day',
                status: order.status, limitPrice: order.limitPrice,
                stopPrice: order.stopPrice,
                filledQty: order.filledQty, filledAvgPrice: order.filledAvgPrice,
                filledAt: order.filledAt ? new Date(order.filledAt) : null,
                reasoning: '外部下单',
              },
            })
      ...
      const progress = baseline > 0 ? ((equity - baseline) / baseline) * 100 : 0
