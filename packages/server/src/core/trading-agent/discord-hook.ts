...
import type { OrderEventNotification, TradeNotification } from './types'
        ...
        embeds: [
            {
                ...
                fields: [
                    { name: 'Qty', value: String(trade.qty), inline: true },
                    {
                        name: 'Price',
                        value:
                            trade.price != null ? `$${trade.price}` : 'pending',
                        inline: true,
                    },
                    {
                        name: 'Reasoning',
                        value: trade.reasoning.slice(0, 1024),
                    },
                ],
...
function formatOrderDescription(n: OrderEventNotification): string {
    ...
    const filled =
        n.filledAvgPrice != null ? ` filled @ $${n.filledAvgPrice}` : ''
    const partial =
        n.filledQty != null && n.filledQty < n.qty
            ? ` ${n.filledQty}/${n.qty}`
            : ` ${n.qty}`
...
export async function notifyOrderEvent(
    ...
    const { label, color } = EVENT_LABELS[n.event] ?? {
        label: n.event,
        color: 0x6b7280,
    }
