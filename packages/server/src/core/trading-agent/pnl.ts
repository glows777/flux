...
export function calculateTradePnl(orders: readonly RawOrder[]): PnlRecord[] {
  ...
  return sorted.map((order): PnlRecord => {
    ...
    if (order.side === 'buy' && order.filledAvgPrice != null && order.filledAt != null) {
      const lots = openLots.get(order.symbol) ?? []
      lots.push({ qty: order.qty, price: order.filledAvgPrice, filledAt: order.filledAt })
      openLots.set(order.symbol, lots)
      return { ...base, realizedPl: null, holdingDays: null }
    }

    if (order.side === 'sell' && order.filledAvgPrice != null && order.filledAt != null) {
      const lots = openLots.get(order.symbol) ?? []
      ...
      while (remainingQty > 0 && lots.length > 0) {
        ...
        totalWeightedDays += daysBetween(lot.filledAt, order.filledAt) * matchQty
      ...
      const holdingDays = order.qty > 0
        ? Math.round(totalWeightedDays / order.qty)
        : 0

      return { ...base, realizedPl: Math.round(totalPl * 100) / 100, holdingDays }
    }
